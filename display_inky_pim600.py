#!/usr/bin/env python3
"""
Display program for the Pimoroni Inky Impression 4" 7-colour HAT (PIM600).

Required files:
- fetcher.py
- display_inky_pim600.py
- data_validator.py
...
def load_fonts():
    try:
        ...
        section_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 15)
        text_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 13)
        small_text_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 10)
    except Exception:
...
def draw_tide_waveform(draw, x, y, width, height, prior_tides_gr, today_tides_gr, next_tides_gr, prior_tides_est, today_tides_est, next_tides_est, prior_jenner_stage_history, today_jenner_stage_history, next_jenner_stage_history, text_font, small_text_font):
    ...
    # Draw axis box
    draw.rectangle((x + margin_left, y + margin_top, x + width - margin_right, y + height - margin_bottom), outline=COLOR_BLACK, fill=COLOR_WHITE)
...
def render_tide_layout(data):
    ...
    if has_tides:
        ...
        draw.rectangle((20, 240, WIDTH - 20, 390), outline=COLOR_BLACK, fill=COLOR_WHITE)
        draw.text((30, 305), "Tide data unavailable", font=text_font, fill=COLOR_BLACK)
...
def main():
    print("Initializing Inky Impression PIM600...")
    ...
    # If inky is available, update dimensions to hardware resolution
    global WIDTH, HEIGHT
    if inky is not None:
        try:
