#!/usr/bin/env python3
"""
Display program for the Pimoroni Inky Impression 4" 7-colour HAT (PIM600) in portrait.

Required files:
- fetcher.py
- display_inky_portrait.py
- data_validator.py
...
- Python 3
- Pillow (PIL)
- inky (required for actual e-ink hardware; optional for PNG output)
...
# Only attempt Inky import on Linux unless explicitly disabled
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print("⚠ Inky library not available, display will save to PNG only")
                    INKY_AVAILABLE = False

# Settings (portrait drawing canvas)
...
def load_and_validate_data(data_file):
    ...
    if not os.path.exists(data_file):
        log_error("Data file missing, creating template", "WARNING")
...
def load_fonts():
    try:
        ...
        small_text_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 12)
        text_font_bold = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 15)
    except Exception:
        header_font = section_font = text_font = small_text_font = ImageFont.load_default()
        text_font_bold = text_font
    return header_font, section_font, text_font, small_text_font, text_font_bold

def draw_flow_block(draw, x, y, data, section_font, text_font, right_edge, title_color=COLOR_BLUE, jenner_color=COLOR_RED, jenner_stage_override=None):
    ...
    draw.text((x, curr_y), "Hacienda Bridge Guernville:", font=text_font, fill=COLOR_BLACK)
    h_bbox = draw.textbbox((0, 0), hacienda_text, font=text_font)
    h_width = h_bbox[2] - h_bbox[0]
    draw.text((right_edge - h_width, curr_y), hacienda_text, font=text_font, fill=COLOR_BLACK)
    curr_y += 22
    # Jenner: label on left, stage right-justified
    jenner_stage_value = jenner_stage_override if jenner_stage_override is not None else data.get('jenner_stage', '--')
    jenner_text = f"{jenner_stage_value} ft"
    ...
    draw.text((x, curr_y), "River Mouth Barrier Bar:", font=text_font, fill=COLOR_BLACK)
...
def draw_coastal_table(draw, x, y, bodega_tides, goat_rock_tides, fort_ross_tides, section_font, text_font, small_text_font, title_color=COLOR_BLUE, goat_rock_color=COLOR_BLUE, bodega_color=COLOR_BLACK, fort_ross_color=COLOR_BLACK, goat_rock_text_font=None):
    ...
    draw.text((col_g_time - 55, header_y), "Goat Rock", font=goat_rock_text_font or text_font, fill=goat_rock_color)
    ...
    for i in range(4):
        ...
        draw.text((col_label, row_y), label, font=text_font, fill=COLOR_BLACK)
        ...
        def draw_right(text, x_right, y_pos, color, font_override=None):
            if not text:
                return
            text = to_24h(text)
            bbox = draw.textbbox((0, 0), text, font=text_font)
            w = bbox[2] - bbox[0]
            draw.text((x_right - w, y_pos), text, font=font_override or text_font, fill=color)

        draw_right(b[1], col_b_time, row_y, bodega_color)
        draw_right(b[2], col_b_ht, row_y, bodega_color)
        draw_right(g[1], col_g_time, row_y, goat_rock_color, font_override=goat_rock_text_font)
        draw_right(g[2], col_g_ht, row_y, goat_rock_color, font_override=goat_rock_text_font)
...
def draw_tide_waveform(draw, x, y, width, height, prior_tides_gr, today_tides_gr, next_tides_gr, prior_tides_est, today_tides_est, next_tides_est, prior_jenner_stage_history, today_jenner_stage_history, next_jenner_stage_history, text_font, small_text_font):
    ...
    if not all_events_gr or len(all_events_gr) < 2 or not all_events_est or len(all_events_est) < 2:
        draw.text((x, y), "Insufficient tide data", font=text_font, fill=COLOR_BLACK)
        return
    ...
    for t_min in range(0, 24 * 60 + 1, step):
        ...
        # For Jenner stage, only draw where we have measured data (curve points not used for plotting)
    ...
    # Plot today's Jenner stage measurements as dots
    if today_jenner_stage_history:
        for measurement in today_jenner_stage_history:
            t_min = measurement.get("minutes")
            stage = measurement.get("stage")
            if t_min is None or stage is None:
                continue
            if 0 <= t_min <= 1440:
                px = x + margin_left + int((t_min / 1440) * graph_width)
                py = y + height - margin_bottom - int(((float(stage) - h_min) / h_range) * graph_height)
                points_jenner_measured.append((px, py))

    # Draw axis box
    draw.rectangle((x + margin_left, y + margin_top, x + width - margin_right, y + height - margin_bottom), outline=COLOR_BLACK, fill=COLOR_LIGHT_BLUE)

    # Warn if prior-day tides missing (can cause flat early-hours)
    if not prior_tides_gr or len(prior_tides_gr) < 2:
        draw.text((x + margin_left + 2, y + margin_top + 2), "Prior tides missing", font=small_text_font, fill=COLOR_BLACK)
        log_error("Prior-day Goat Rock tides missing; early hours may appear flat", "WARNING")

    # Warn if stage data appears flat
    if stage_flat:
        draw.text((x + margin_left + 2, y + margin_top + 14), "Stage flat", font=small_text_font, fill=COLOR_BLACK)

    # Draw Goat Rock curve (solid line)
    if len(points_gr) > 1:
        for i in range(len(points_gr) - 1):
            draw.line((points_gr[i], points_gr[i + 1]), fill=COLOR_BLUE, width=6)

    # Estuary curve calculation retained, but not drawn

    # Draw Jenner Stage measurements as large dots
    if points_jenner_measured:
        radius = 4
        for px, py in points_jenner_measured:
            draw.ellipse((px - radius, py - radius, px + radius, py + radius), fill=COLOR_RED, outline=COLOR_BLACK)
    ...
    for h_label in [-2, 0, 2, 4, 6, 8, 10]:
        ...
        # Draw tick mark
        draw.line((x + margin_left - 2, py, x + margin_left, py), fill=COLOR_BLACK, width=1)
        ...
        draw.text((x + margin_left - 6 - label_width, py - 2), label_text, font=small_text_font, fill=COLOR_BLACK)

    # Draw x-axis time labels (4:00, 8:00, 12:00, 16:00, 20:00)
    for h_label in [4, 8, 12, 16, 20]:
        t_min = h_label * 60
        px = x + margin_left + int((t_min / 1440) * graph_width)
        py = y + height - margin_bottom + 2
        # Draw tick mark
        draw.line((px, y + height - margin_bottom, px, y + height - margin_bottom + 3), fill=COLOR_BLACK, width=1)
        # Draw label with :00 format
        label = f"{h_label}:00"
        draw.text((px - 12, py), label, font=small_text_font, fill=COLOR_BLACK)

def render_tide_layout(data):
    # Create Image
    img = Image.new("RGB", (WIDTH, HEIGHT), COLOR_WHITE)
    draw = ImageDraw.Draw(img)
    header_font, section_font, text_font, small_text_font, text_font_bold = load_fonts()

    # 1. Header Bar with data status
    draw.rectangle((0, 0, WIDTH, 40), fill=COLOR_BLUE)
    ...
    draw.text(((WIDTH - (bbox[2] - bbox[0])) // 2, 6), f"{date_str}", font=header_font, fill=COLOR_YELLOW)
    ...
    jenner_stage_override = None
    today_jenner_stage = data.get("jenner_stage_history", {}).get(today_key, [])
    if today_jenner_stage:
        # Log last few stage values for diagnostics
        last_few = today_jenner_stage[-5:]
        for m in last_few:
            stage_val = m.get("stage")
            try:
                float_stage = float(stage_val)
                log_error(f"Jenner stage sample: raw={stage_val} float={float_stage}", "INFO")
            except Exception:
                log_error(f"Jenner stage sample: raw={stage_val} (non-numeric)", "WARNING")

        for measurement in reversed(today_jenner_stage):
            if measurement.get("stage") is not None:
                jenner_stage_override = measurement.get("stage")
                break
    draw_flow_block(draw, 20, 56, data, section_font, text_font, WIDTH - 20, title_color=COLOR_BLUE, jenner_color=COLOR_RED, jenner_stage_override=jenner_stage_override)

    # 3. Block 2: Coastal table (Bodega, Goat Rock, Fort Ross)
    draw_coastal_table(draw, 14, 190, bodega_tides, goat_rock_tides, fort_ross_tides, section_font, text_font, small_text_font, title_color=COLOR_BLUE, goat_rock_color=COLOR_BLUE, goat_rock_text_font=text_font_bold)
    ...
    subtitle = "(Goat Rock & Stage)"
    sub_bbox = draw.textbbox((0, 0), subtitle, font=small_text_font)
    sub_w = sub_bbox[2] - sub_bbox[0]
    draw.text(((WIDTH - tide_w) // 2, 330), tide_title, font=section_font, fill=COLOR_BLUE)
    draw.text(((WIDTH - sub_w) // 2, 355), subtitle, font=small_text_font, fill=COLOR_BLACK)
    ...
    if has_tides:
        draw_tide_waveform(
            ...
            estuary_tides,
            tomorrow_tides_est,
            yesterday_jenner_stage,
            today_jenner_stage,
            tomorrow_jenner_stage,
            text_font,
            small_text_font,
        )
    else:
        # Graceful degradation: show message instead of curves
        draw.rectangle((14, 375, WIDTH - 14, 625), outline=COLOR_BLACK, fill=COLOR_LIGHT_BLUE)
        draw.text((24, 490), "Tide data unavailable", font=text_font, fill=COLOR_BLACK)
        draw.text((24, 515), "Check data file", font=small_text_font, fill=COLOR_BLACK)
...
def main():
    print("Initializing Inky Impression PIM600 (Portrait)...")
    ...
    if INKY_AVAILABLE:
        try:
            ...
            log_error(f"Failed to initialize Inky Impression: {e}", "WARNING")
            inky = None

    # If inky is available, update dimensions to portrait drawing canvas
    global WIDTH, HEIGHT
    if inky is not None:
        try:
            WIDTH, HEIGHT = inky.height, inky.width
        except Exception:
            pass
